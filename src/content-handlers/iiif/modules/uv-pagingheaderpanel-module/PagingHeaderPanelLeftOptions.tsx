...
const PagingHeaderPanelLeftOptions: React.FC<Props> = ({ extensionHost, extension, content, pageMode, onClick }) => {
  type CanvasItem = { label: string | null; index: number };
  const [canvasItems, setCanvasItems] = useState<CanvasItem[]>([]);

  const searchEnabled: boolean = (extension as OpenSeadragonExtension)?.isSearchEnabled() ?? false;
